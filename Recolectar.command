#!/bin/bash
# Doble clic para RECOLECTAR datos de entrenamiento:
#   1) primera pantalla: elegir cámara (salen todas; clic en la que quieras)
#   2) al elegir, esa MISMA pestaña se redirige al panel web VisionMetrics
#      (vista en vivo + etiquetado + entrenar un modelo con esta sesión)
# Equivalente en Mac de Recolectar.bat (Windows). Portátil + auto-instala la 1ª vez.
cd "$(dirname "$0")" || exit 1
...
venv/bin/python run.py --recolectar
